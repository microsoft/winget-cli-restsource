...
namespace Microsoft.WinGet.Functions.Functions
{
    ...
    public static class ManifestFunctions
    {
        ...
        /// This allows us to make post requests for manifests.
        /// </summary>
        /// <param name="req">HttpRequest.</param>
        /// <param name="client">CosmosDB DocumentClient.</param>
            ...
            ILogger log)
        {
            ...
            {
                manifest = await Parser.StreamParser<Manifest>(req.Body, log);
                Uri documentUri = UriFactory.CreateDocumentCollectionUri(Constants.CosmosConnectionConstants.DatabaseName, Constants.CosmosConnectionConstants.CollectionName);
                await client.UpsertDocumentAsync(documentUri, manifest);
            }
            catch (Exception e)
            {
                log.LogError(e.ToString());
            ...
            ILogger log)
        {
            ...
            try
            {
                ...
                IDocumentQuery<Manifest> query = client.CreateDocumentQuery<Manifest>(collectionUri)
                    .AsDocumentQuery();
