...
class WinGetInstaller
{
    ...
    [WinGetInstaller[]] Concat ([string] $a)
    {
        $Converted = convertfrom-Yaml -yaml $a
        $ReturnResult = [WinGetInstaller[]]::new("")

        foreach($Installer in ($a.Installers)) {
            $Temp = [WinGetInstaller]::new("")

            $Temp.InstallerIdentifier       = "$($Installer.Architecture) $($Installer.Scope)"
            $Temp.InstallerSha256           = $Installer.InstallerSha256
            $Temp.InstallerUrl              = $Installer.InstallerUrl
            $Temp.Architecture              = $Installer.Architecture
            $Temp.InstallerLocale           = $Converted.InstallerLocale
            $Temp.Platform                  = $Converted.Platform
            $Temp.MinimumOsVersion          = $Converted.MinimumOsVersion
            $Temp.InstallerType             = $Converted.InstallerType
            $Temp.Scope                     = $Installer.Scope
            $Temp.SignatureSha256           = $Converted.SignatureSha256
            $Temp.InstallModes              = $Converted.InstallModes
            $Temp.InstallerSwitches         = $Converted.InstallerSwitches.Silent
            $Temp.InstallerSuccessCodes     = $Converted.InstallerSuccessCodes
            $Temp.ExpectedReturnCodes       = $Converted.ExpectedReturnCodes
            $Temp.UpgradeBehavior           = $Converted.UpgradeBehavior
            $Temp.Commands                  = $Converted.Commands
            $Temp.Protocols                 = $Converted.Protocols
            $Temp.FileExtensions            = $Converted.FileExtensions
            $Temp.Dependencies              = $Converted.Dependencies
            $Temp.PackageFamilyName         = $Converted.PackageFamilyName
            $Temp.ProductCode               = $Converted.ProductCode
            $Temp.Capabilities              = $Converted.Capabilities
            $Temp.RestricedCapabilities     = $Converted.RestricedCapabilities
            $Temp.MSStoreProductIdentifier  = $Converted.MSStoreProductIdentifier
            $Temp.InstallerAbortsTerminal   = $Converted.InstallerAbortsTerminal
            $Temp.ReleaseDate               = $Converted.ReleaseDate
            $Temp.InstallLocationRequired   = $Converted.InstallLocationRequired
            $Temp.RequireExplicitUpgrade    = $Converted.RequireExplicitUpgrade
            $Temp.ElevationRequirement      = $Converted.ElevationRequirement
            $Temp.UnsupportedOSArchitectures= $Converted.UnsupportedOSArchitectures
            $Temp.AppsAndFeaturesEntries    = $Converted.AppsAndFeaturesEntries
            $Temp.Markets                   = $Converted.Markets
            $Temp.DownloadCommandProhibited = $Converted.DownloadCommandProhibited
